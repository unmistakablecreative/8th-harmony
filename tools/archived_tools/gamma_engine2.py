...
INPUT_FILE = os.path.join(BASE_DIR, "gamma_input.txt")
...
def poll_until_ready(gamma_id, timeout=90, interval=5):
    ...
    for _ in range(0, timeout, interval):
        try:
            response = requests.get(poll_url, headers={"X-API-KEY": GAMMA_API_KEY})
            data = response.json()
...
def main():
    ...
    parser.add_argument("--params", help=argparse.SUPPRESS)  # avoid orchestrate crash
